...
import com.grapecity.xuni.gauge.XuniGauge;
import com.grapecity.xuni.gauge.XuniRadialGauge;
import com.grapecity.xuni.samples.flexgrid.data.Customer;

/**
 * Custom GridCellFactory to show how to inject views into a cell to fully customize the content.
 * 
 * @author GrapeCity
 * 
 */
public class CustomPerformanceCellFactory extends GridCellFactory
{
	...
	public CustomPerformanceCellFactory(FlexGrid flexGrid, Context context)
	{
		super(flexGrid);
	...
	public void createCellContent(GridPanel gridPanel, FlexGridCanvasRenderEngine renderEngine, GridCellRange cellRange, Rect bounds)
	{
		...
		// condition to check cells under column Order Count
		if (gridPanel.getCellType().equals(GridCellType.CELL) && gridPanel.getColumns().get(cellRange.col).getName().equals("Order Count"))
		{
			// getting data object
			...
			mPerformanceGauge.setValue(mCustomer.getOrderCount());
